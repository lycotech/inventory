...
export async function POST(req: Request) {
  ...
  // Role-based access control: only admin and manager can record stock out
  ...
  if (decimalCompare(inv.stockQty, absQty).isLess) {
    ...
    try {
      const created = await prisma.alertLog.create({
        data: {
          ...
          priorityLevel: "high",
          message: `Attempted to stock out ${absQty} but only ${inv.stockQty} available for ${inv.itemName} (${inv.barcode}) at ${inv.warehouseName}`,
          inventoryId: inv.id,
          acknowledged: false,
        },
      });
    ...
    if (preventNegative) {
      return NextResponse.json({ 
        error: "Stock out quantity would result in negative stock. Reduce the quantity." 
      }, { status: 400 });
    }
    // else: proceed with stock out (will make stock negative)
  }
  ...
  const tx = await prisma.$transaction(async (db: Parameters<typeof prisma.$transaction>[0] extends (arg: infer A) => any ? A : any) => {
    ...
    // Low stock alert if we crossed threshold after stock out
    try {
      if (decimalToNumber(updated.stockAlertLevel) > 0 && decimalCompare(updated.stockQty, updated.stockAlertLevel).isLessOrEqual) {
        const createdLow = await db.alertLog.create({
          data: {
            ...
            message: `Low stock after sale: ${updated.itemName} (${updated.barcode}) at ${updated.warehouseName} — ${decimalToNumber(updated.stockQty)} <= alert ${decimalToNumber(updated.stockAlertLevel)}`,
